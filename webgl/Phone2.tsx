...
export default function Phone2(props: any) {

  const [showPhone, setShowPhone] = useState(false)
  const [playMusic, setPlayMusic] = useState(false)

  const gltf: any = useLoader(GLTFLoader, '/assets/models/phone/phone2.glb', (loader: any) => {
  ...
  return (
    <>
      <mesh ref={phoneRef} visible={props.visible}>
        <primitive object={gltf.scene} position={props.position} scale={props.scale} rotation={[0.0, Math.PI / 6, 0]}>
          <Html position={[0, 0.18, 0]} style={{ display: showPhone ? 'block' : 'none' }}>
            <div style={{ transform: 'translate3d(-50%, -117%, 0px)' }}>
              <div
                ...
                onClick={(e) => useStore.setState({ popup: 4 })}
              >
                <div
                  style={{
                    width: '32px',
                ...
                <audio autoPlay loop muted={playMusic ? false : true} id="audioDetail">
                  <source src="/assets/media/Calm-and-Peaceful.mp3" type="audio/ogg" />
                </audio>
              </div>
            </div>
          </Html>
